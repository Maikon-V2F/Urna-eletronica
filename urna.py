...
def exibirResultados(total_eleitores, total_votos_nominais, total_brancos, total_nulos, votos_candidatos):
    ...
    for cargo, votos_candidato in votos_candidatos.items():
        ...
        total_votos_cargo = sum(votos_candidato.values())
        for candidato, quantidade in votos_candidato.items():
            porcentagem = (quantidade / total_votos_cargo) * 100
            print(
                f"Candidato: {candidato} | Votos: {quantidade} ({porcentagem:.2f}%)"
            )

def gera_grafico(titulo, votos_candidato):
    candidato_nomes = list(votos_candidato.keys())
    quantidade_votos = list(votos_candidato.values())

    plt.bar(candidato_nomes, quantidade_votos, color='blue')
    plt.title(titulo)
    plt.xlabel('Candidatos')
    plt.ylabel('Quantidade de Votos')

    # Ajusta o espaçamento dos nomes dos candidatos no eixo x para evitar sobreposição
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()  # Ajusta automaticamente o layout para evitar cortar rótulos
...
def receberOpcao():
    ...
    if opcaoRecebida == 1:
        ...
        total_eleitores, total_votos_nominais, total_brancos, total_nulos, votos_candidatos = apurarVotos()
        for cargo, votos_candidato in votos_candidatos.items():
            gera_grafico(f"Resultado da Eleição para {cargo}", votos_candidato)
